...
export default function SearchJobs({ user, token }: any) {
  ...
  useEffect(() => {
    if (debouncedSearch) {
      axios
        .get(
          `${process.env.NEXT_PUBLIC_SERVER_LINK}/explores/search-jobs?query=${query}`,
          {
            headers: {
  ...
  return (
    <>
      <div className="w-full flex relative">
        ...
        />
        <div className="bg-transparent  rounded-r bg-teal-200 absolute top-0 bottom-0 right-0 px-6 text-teal-600 flex items-center justify-center rounded-r">
          <svg
